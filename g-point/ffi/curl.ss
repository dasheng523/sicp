...
(library (ffi curl)
  ...
  (define-enum-ftype CURLUSESSL-t
    (NONE TRY CONTROL ALL LAST)
    int)

  (define-enum-ftype CURLcode-t
    (CURLE_OK
  ...
  (define (curl-easy-perform curl)
    ...
    (define r (f curl))
    (unless (= r CURLE_OK)
      (error 'curl-easy-perform "failed" r)))

  (define-ftype curl-read-callback
    (function (void* size_t size_t scheme-object) size_t))

  (define-ftype curl-write-callback
    (function (u8* size_t size_t scheme-object) size_t))
