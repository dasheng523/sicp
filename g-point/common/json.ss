(library (common json)
                ...
                (convert-pattern
                 (and-pattern
                  ...
                  (join-pattern (and-pattern
                                 (filter-blank-pattern (convert-pattern string-pattern string->symbol))
                                 (skip-blank-simple-pattern ":")
  ...
  (define (test-json->string json result)
  ...
  (define (json->string json)
    (cond
     ...
     [(eq? json #f) "false"]
     [(eq? json 'nil) "null"]
     [(number? json) (->string json)]
     [(string? json) (string-append "\"" json "\"")]
     [(json-object? json)
      (string-append "{"
                     (string-join
                      (map (lambda (item)
                             (string-append (json->string (->string (car item)))
                                            ":"
                                            (json->string (cdr item))))
                           json)
                      ",")
                     "}")]
     [(json-array? json)
  ...
  (test-json->string 888 "888")
  (test-json->string "666" "\"666\"")
  (test-json->string (list
                      (cons 'a "666")
                      (cons 'b 'nil)
                      (cons 'c #f)
                      (cons 'd 8))
                     "{\"a\":\"666\",\"b\":null,\"c\":false,\"d\":8}")
  (test-json->string (list
                      (cons 'a "666")
                      (cons 'b (list
                                (cons 'aa 'nil)
                                (cons 'bb 0))))
                     "{\"a\":\"666\",\"b\":{\"aa\":null,\"bb\":0}}")
  (test-json->string (list
                      (cons 'a "666")
                      (cons 'b 'nil)
                      (cons 'c #f)
  ...
  #;(let* ([a (json->string
            (read-string-with-file "e:/aaa.txt" string->json))]
        [b (json->string (string->json a))])
  (string=? a b))

  #; (write-string-file "e:/aaa1.txt" (json->string
                                    (read-string-with-file "e:/aaa.txt" string->json)))
)
